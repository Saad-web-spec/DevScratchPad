...
const TOOL_MAPPINGS: { keywords: string[]; slugs: string[] }[] = [
  {
    keywords: ["next", "react", "vue", "svelte", "typescript", "frontend", "tailwind"],
    slugs: ["json-to-ts", "json-to-zod", "svg-to-jsx", "jwt"],
  },
  {
    keywords: ["fastapi", "django", "python", "go", "rust", "backend", "api", "hono"],
    slugs: ["curl-to-fetch", "curl-to-python", "json-formatter", "hmac-generator"],
  },
  {
    keywords: ["postgres", "sqlite", "database", "prisma", "sql"],
    slugs: ["sql-formatter", "uuid-generator", "json-formatter", "hash"],
  },
  {
    keywords: ["docker", "devops", "kubernetes", "terraform"],
    slugs: ["yaml", "cron", "cidr-calculator", "base64-inspector"],
  },
];

function getContextualTools(techName: string, category: string) {
  const query = `${techName} ${category}`.toLowerCase();
  for (const m of TOOL_MAPPINGS) {
    if (m.keywords.some((k) => query.includes(k))) {
      return m.slugs.map((s) => TOOLS_REGISTRY[s]).filter(Boolean);
    }
  }
  return ["json-formatter", "jwt", "json-to-ts", "diff"]
  ...
  return (
    <article className="w-full bg-white border-t border-zinc-200 mt-8 py-16 px-4 sm:px-6 lg:px-8 text-zinc-800 font-sans selection:bg-zinc-900 selection:text-white">
      <div className="max-w-7xl mx-auto space-y-16">
        ...
        {/* 6. Recommended Offline Developer Tools (Reciprocal Cross-Linking) */}
        <section className="space-y-4 pt-4 border-t border-zinc-200">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
            <div>
              <h3 className="text-lg font-bold text-zinc-900 flex items-center gap-2">
                ...
                <span>Pair With {route.techName} Client-Side Utilities</span>
              </h3>
            ...
            <Link
              ...
              className="text-xs font-mono font-semibold text-orange-600 hover:text-orange-700 inline-flex items-center gap-1 shrink-0"
            >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            {getContextualTools(route.techName, route.category).map((tool) => (
              <Link
                key={tool.slug}
                href={`/tools/${tool.slug}`}
                className="p-3.5 rounded-xl bg-white border border-zinc-200 hover:border-orange-400 hover:bg-orange-50/20 transition-all shadow-2xs group flex flex-col justify-between"
              >
                <div>
                  <span className="text-[9px] font-mono uppercase text-zinc-400 block mb-1">
                    {tool.category.split(" ")[0]}
                  </span>
                  <h4 className="text-xs font-bold text-zinc-900 group-hover:text-orange-700 transition-colors">
                    {tool.name}
                  </h4>
                ...
                <span className="text-[10px] font-mono text-orange-600 font-semibold mt-2.5 inline-flex items-center gap-1 group-hover:translate-x-0.5 transition-transform">
                  <span>Open Tool</span>
                  <span>&rarr;</span>
                </span>
              </Link>
        ...
        {/* 7. Stack-specific Frequently Asked Questions */}
