  ...
  return (
    <div className="flex flex-col h-full bg-white dark:bg-slate-950 w-full overflow-x-hidden">
      ...
      <div className="min-h-14 border-b border-[#e2e8f0] dark:border-slate-800 flex flex-wrap md:flex-nowrap items-center justify-between px-3 md:px-4 py-2 md:py-0 bg-[#f8fafc] dark:bg-slate-900 shrink-0 gap-2">
        <div>
          <h2 className="text-sm font-semibold text-slate-800 dark:text-slate-200">Regex Tester</h2>
          <p className="text-[11px] text-slate-400 dark:text-slate-500 hidden sm:block">Test regular expressions and inspect match groups</p>
        </div>
      ...
      <div className="border-b border-[#e2e8f0] dark:border-slate-800 bg-white dark:bg-slate-950 p-3 px-3 md:px-4 flex flex-col gap-2 shrink-0">
        <div className="flex items-center bg-[#f8fafc] dark:bg-slate-900 border border-slate-200 dark:border-slate-700 focus-within:border-blue-500 rounded-lg px-2.5 py-1.5">
          <span className="text-slate-400 dark:text-slate-500 font-mono text-sm md:text-base font-semibold select-none pr-1">
            /
          </span>
          <input
            type="text"
            ...
            className="flex-1 min-w-0 bg-transparent text-slate-900 dark:text-slate-100 font-mono text-xs md:text-sm focus:outline-none placeholder:text-slate-400 dark:placeholder:text-slate-500"
          />
          <span className="text-slate-400 dark:text-slate-500 font-mono text-xs md:text-base font-semibold select-none pl-1 shrink-0">
            /{activeFlagsString}
          </span>
        </div>
        ...
        {replaceMode && (
          <div className="flex items-center bg-[#f8fafc] dark:bg-slate-900 border border-slate-200 dark:border-slate-700 focus-within:border-blue-500 rounded-lg px-2.5 py-1.5 mt-1">
            <span className="text-xs font-medium text-slate-500 dark:text-slate-400 pr-2 select-none shrink-0">
              Replace:
            </span>
            <input
              type="text"
              ...
              className="flex-1 min-w-0 bg-transparent text-slate-900 dark:text-slate-100 font-mono text-xs focus:outline-none placeholder:text-slate-400 dark:placeholder:text-slate-500"
            />
          </div>
        )}
              ...
              <div className="flex-1 overflow-y-auto p-3 md:p-4 space-y-3">
                {matches.length === 0 ? (
                  <div className="flex flex-col items-center justify-center h-full text-slate-400 dark:text-slate-500 py-12">
                    <RegexIcon className="w-8 h-8 text-slate-300 dark:text-slate-600 mb-2" />
                    <p className="text-xs">
                      {pattern
                  ...
                  matches.map((m, idx) => (
                    <div
                      key={idx}
                      className="border border-[#e2e8f0] dark:border-slate-800 rounded-lg p-3 bg-[#f8fafc]/50 dark:bg-slate-900/50 hover:bg-[#f8fafc] dark:hover:bg-slate-900 transition-colors group"
                    >
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <button
                          ...
                          className={cn(
                            "flex items-center gap-1 text-[11px] px-2 py-1 rounded bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 transition-colors",
                            copied === `match-${idx}`
                      ...
                      <div className="p-2 bg-white dark:bg-slate-950 rounded border border-[#e2e8f0] dark:border-slate-800 font-mono text-xs text-slate-800 dark:text-slate-200 break-all select-all mb-2">
                        {m.match}
                      </div>
                      ...
                      {m.captures && m.captures.length > 0 && (
                        <div className="mt-2 space-y-1.5 pl-2 border-l-2 border-blue-200 dark:border-blue-800">
                          ...
                          {m.captures.map((cap, capIdx) => (
                            ...
                            >
                              <span className="font-mono text-slate-400 dark:text-slate-500 shrink-0 text-[11px]">
                                ${capIdx + 1}:
                              </span>
                              <span className="font-mono text-slate-700 dark:text-slate-300 bg-white dark:bg-slate-950 px-1.5 py-0.5 rounded border border-slate-200 dark:border-slate-800 break-all">
