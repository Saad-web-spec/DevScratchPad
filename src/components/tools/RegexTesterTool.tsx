    ...
    <div className="flex flex-col h-full bg-[#09090B] w-full overflow-hidden relative">
      ...
      <div className="min-h-14 border-b border-zinc-200 dark:border-zinc-800 flex items-center justify-between gap-2 overflow-x-auto no-scrollbar py-2 px-3 md:px-4 bg-white dark:bg-zinc-950 shrink-0 sticky top-0 z-10">
        <div className="flex items-center gap-2">
          <RegexIcon className="w-4 h-4 text-zinc-900 dark:text-zinc-100" />
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-6 lg:p-8 w-full mx-auto bg-white dark:bg-[#09090B] flex flex-col max-w-[1400px]">
        {/* Integrated Regex Pattern Input Bar */}
        <div className="bg-zinc-50 dark:bg-zinc-900/50 border border-zinc-200 dark:border-zinc-800 rounded-xl p-3 shadow-xs flex items-center justify-between gap-3 focus-within:border-zinc-400 dark:focus-within:border-zinc-600 transition-colors">
          ...
          <input
            ...
            spellCheck={false}
            className="font-mono text-sm md:text-base tracking-wide bg-transparent border-0 focus:ring-0 focus:outline-none text-zinc-900 dark:text-zinc-100 flex-1 placeholder:text-zinc-400 dark:placeholder:text-zinc-600 min-w-0"
          />
          <span className="text-zinc-400 font-mono font-bold select-none text-sm md:text-base pr-1">/</span>

          {/* Interactive Flag Toggles */}
          <div className="bg-zinc-100 dark:bg-zinc-800 p-1 rounded-lg flex items-center gap-0.5 shrink-0 ml-2">
            {[
              { id: "g", label: "g" },
              { id: "i", label: "i" },
              { id: "m", label: "m" },
              ...
              <button
                ...
                className={cn(
                  ...
                  flags[f.id]
                    ? "bg-zinc-900 text-white dark:bg-zinc-100 dark:text-zinc-900 font-bold rounded shadow-xs"
        ...
        {replaceMode && (
          <div className="bg-zinc-50 dark:bg-zinc-900/50 border border-zinc-200 dark:border-zinc-800 rounded-xl p-3 shadow-xs flex items-center justify-between gap-3 mt-3 focus-within:border-zinc-400 dark:focus-within:border-zinc-600 transition-colors">
            <span className="text-xs font-semibold text-zinc-400 dark:text-zinc-500 uppercase tracking-wider select-none shrink-0 pl-1">
              Replace:
            </span>
            <input
              type="text"
        ...
        {regexError && (
          <div className="flex items-center gap-2 text-xs text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/20 px-3.5 py-2 rounded-lg mt-3">
        ...
        <div className="flex lg:hidden bg-zinc-50 dark:bg-zinc-900/50 p-2 border-b border-zinc-200 dark:border-zinc-800 shrink-0 mt-4 rounded-xl">
          <div className="bg-zinc-200/50 dark:bg-zinc-950/50 p-1 rounded-lg flex items-center w-full">
            <button
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-6 mt-4 lg:h-[calc(100vh-220px)] min-h-[500px]">
          ...
          <div
            className={cn(
              "border border-zinc-200 dark:border-zinc-800 rounded-xl bg-zinc-50 dark:bg-zinc-900/40 flex flex-col p-4 shadow-sm",
          ...
          >
            ...
            <div className="flex-1 relative w-full h-full min-h-[200px] border border-zinc-200 dark:border-zinc-800 rounded-lg overflow-hidden bg-white dark:bg-[#09090B]">
              <MonacoEditor
                height="100%"
          ...
          <div
            className={cn(
              "border border-zinc-200 dark:border-zinc-800 rounded-xl bg-zinc-50 dark:bg-zinc-900/40 p-4 shadow-sm flex flex-col",
              ...
              <>
                <div className="flex items-center justify-between mb-3 shrink-0">
                  ...
                  <button
                    ...
                    className="text-zinc-400 hover:text-zinc-900 dark:hover:text-zinc-100 transition-colors flex items-center gap-1 p-1"
                  >
                    {copied === "replaced" ? <Check className="w-3.5 h-3.5 text-zinc-900 dark:text-zinc-100" /> : <Copy className="w-3.5 h-3.5" />}
                  </button>
                </div>
                <div className="flex-1 relative w-full h-full min-h-[200px] border border-zinc-200 dark:border-zinc-800 rounded-lg overflow-hidden bg-white dark:bg-[#09090B]">
                  <MonacoEditor
              ...
              <>
                <div className="flex items-center justify-between mb-3 shrink-0">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="bg-zinc-200 dark:bg-zinc-800 text-zinc-800 dark:text-zinc-200 border border-zinc-300 dark:border-zinc-700 text-xs font-mono px-2.5 py-0.5 rounded-full">
                      {matches.length}
                    </span>
                  </div>
                  {matches.length > 0 && (
                    <button
                      ...
                      className="text-zinc-400 hover:text-zinc-900 dark:hover:text-zinc-100 transition-colors flex items-center gap-1.5 p-1 text-xs font-medium"
                    >
                      {copied === "all-matches" ? <Check className="w-3.5 h-3.5 text-zinc-900 dark:text-zinc-100" /> : <Copy className="w-3.5 h-3.5" />}
                ...
                <div className="flex-1 overflow-y-auto space-y-3 pr-1">
                  {matches.length > 0 ? (
                    matches.map((m, idx) => (
                      <div key={idx} className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg p-3.5 shadow-xs relative group hover:border-zinc-300 dark:hover:border-zinc-700 transition-colors">
                        
                        <div className="flex items-center justify-between mb-2">
                          <div className="flex items-center">
                            <span className="bg-zinc-900 text-white dark:bg-zinc-100 dark:text-zinc-900 text-[11px] font-mono font-bold px-2 py-0.5 rounded">
                              Match {idx + 1}
                            </span>
                          ...
                          <button
                            ...
                            className="h-7 px-2 text-xs bg-zinc-100 dark:bg-zinc-800 hover:bg-zinc-200 dark:hover:bg-zinc-700 text-zinc-700 dark:text-zinc-300 rounded transition-colors flex items-center gap-1 opacity-0 group-hover:opacity-100 focus:opacity-100"
                            title="Copy Match"
                          >
                        ...
                        <div className="font-mono text-sm text-zinc-900 dark:text-zinc-100 bg-zinc-50 dark:bg-zinc-950 p-2.5 rounded-md border border-zinc-200/60 dark:border-zinc-800/80 mt-2 select-all break-all whitespace-pre-wrap">
                          {m.match}
                        </div>
                        ...
                        {m.captures && m.captures.length > 0 && (
                          <div className="mt-3 flex flex-wrap gap-2">
                            {m.captures.map((g, gIdx) => (
                              <div key={gIdx} className="flex items-center gap-1.5 bg-zinc-100 dark:bg-zinc-800/60 px-2 py-1 rounded border border-zinc-200 dark:border-zinc-700/50">
                                <span className="font-mono text-[10px] text-zinc-400 font-bold tracking-wider">${gIdx + 1}</span>
                                <span className="font-mono text-xs text-zinc-700 dark:text-zinc-300 select-all truncate max-w-[200px]">{g}</span>
                              </div>
                            ))}
                          </div>
                        )}
                    ...
                    <div className="h-full flex flex-col items-center justify-center text-center text-zinc-500 dark:text-zinc-400 p-8">
