...
export function CronVisualizerTool({
 ...
 <span className="text-sm font-semibold text-zinc-800">Cron Expression Visualizer</span>
 </div>

 <div className="flex items-center gap-2 flex-wrap">
 <ValidationBadge isValid={isValid} />
 <ExportImageButton code={output || input} language="plaintext"/>
 <EmbedButton toolSlug="cron-visualizer"data={input} />
...
"px-3 py-1.5 rounded-full text-xs font-medium transition-colors border",
 ...
 <label htmlFor="cron-expression-input"className="text-xs font-semibold text-zinc-700 uppercase tracking-wider">
 Cron Expression
 </label>
 <button
 onClick={() => setInput("")}
 ...
 return (
 <div key={field.name} className="flex flex-col items-center text-center p-3 md:p-4 rounded-lg bg-white ] border border-[#E2E8F0] ]">
 <span className="text-[10px] text-zinc-500 font-semibold uppercase tracking-wider mb-1">{field.name}</span>
 <span className="font-mono text-base md:text-lg font-bold text-[#2563EB] mb-1">{val}</span>
 ...
 {output && (
 <div className="w-full p-6 bg-white ] border border-[#E2E8F0] ] rounded-xl relative flex flex-col md:flex-row md:items-start justify-between gap-4">
 <div className="flex items-start gap-3 md:gap-4">
 <div className="shrink-0 mt-0.5">
 <Clock className="w-6 h-6 md:w-7 md:h-7 text-[#0F172A]"strokeWidth={1.5} />
 </div>
 <div className="min-w-0">
 <h3 className="text-lg md:text-xl font-semibold text-zinc-900 leading-snug break-words">
 {output}
 </h3>
 <p className="text-xs text-zinc-500 mt-2 font-mono break-all">
 Expression: <span className="text-zinc-800 font-semibold">{input}</span>
 </p>
 </div>
 </div>
...
"h-9 px-3 bg-white hover:bg-zinc-100 border border-zinc-200 text-zinc-700 text-xs font-medium rounded-md transition-colors flex items-center gap-1.5",
 ...
 >
 {copied ? <Check className="w-3.5 h-3.5 text-zinc-900"/> : <Copy className="w-3.5 h-3.5 text-zinc-400"/>}
