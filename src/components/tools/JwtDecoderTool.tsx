      ...
      {entries.map(([key, val], i) => {
        ...
        if (isTimeClaim) {
          try {
            ...
            badge = (
              <span className="ml-2 inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-sans font-medium bg-zinc-200 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400">
...
export function JwtDecoderTool({ onValidationChange, onStatsChange, restoredInput }: JwtDecoderToolProps) {
  ...
  return (
    <div className="flex flex-col h-full bg-[#09090B] w-full overflow-hidden">
      {/* Tool Header */}
      <div className="min-h-14 border-b border-zinc-200 dark:border-zinc-800 flex items-center justify-between gap-2 overflow-x-auto no-scrollbar py-2 px-3 md:px-4 bg-white dark:bg-zinc-950 shrink-0">
        <div className="flex items-center gap-2">
          <Key className="w-4 h-4 text-zinc-900 dark:text-zinc-100" />
          <h2 className="text-sm font-semibold text-zinc-900 dark:text-zinc-100">JWT Decoder</h2>
        </div>
      ...
      <div className="flex lg:hidden bg-zinc-50 dark:bg-zinc-900/50 p-2 border-b border-zinc-200 dark:border-zinc-800 shrink-0">
      ...
      {/* Main Canvas Area */}
      <div className="flex-1 overflow-y-auto p-4 md:p-6 bg-white dark:bg-[#09090B] flex flex-col">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-6 lg:h-[calc(100vh-140px)] min-h-[600px]">
          
          {/* Left Panel: Encoded Input */}
          <div className={cn("border border-zinc-200 dark:border-zinc-800 rounded-xl bg-zinc-50 dark:bg-zinc-900/50 flex flex-col overflow-hidden h-full", activeTab !== "input" && "hidden lg:flex")}>
            <div className="h-10 bg-zinc-100 dark:bg-zinc-900/80 border-b border-zinc-200 dark:border-zinc-800 flex items-center justify-between px-4 shrink-0">
              <span className="text-[11px] font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">Encoded JWT</span>
              <div className="flex items-center gap-2">
                <button onClick={() => setInput("")} className="text-zinc-400 hover:text-red-500 transition-colors p-1" title="Clear">
                  <Trash2 className="w-3.5 h-3.5" />
                </button>
                <button onClick={() => handleCopy(input, 'input')} className="text-zinc-400 hover:text-zinc-900 dark:hover:text-zinc-100 transition-colors p-1">
                  {copied === 'input' ? <Check className="w-3.5 h-3.5" /> : <Copy className="w-3.5 h-3.5" />}
                </button>
              </div>
            </div>
            
            <div className="flex-1 relative bg-white dark:bg-[#09090B]">
              <div className="absolute inset-0 p-4 font-mono text-[13px] sm:text-sm break-all whitespace-pre-wrap pointer-events-none leading-[1.6]">
                ...
                {payloadPart !== null && <span className="text-zinc-400 dark:text-zinc-600">.</span>}
                {payloadPart !== null && <span className="text-purple-500 font-medium">{payloadPart}</span>}
                {signaturePart !== null && <span className="text-zinc-400 dark:text-zinc-600">.</span>}
              ...
              <textarea 
                ...
                className="absolute inset-0 p-4 w-full h-full font-mono text-[13px] sm:text-sm bg-transparent text-transparent caret-zinc-900 dark:caret-zinc-100 resize-none outline-none break-all leading-[1.6] placeholder:text-zinc-400 dark:placeholder:text-zinc-600"
              />
            </div>
          ...
          <div className={cn("flex flex-col gap-4 overflow-y-auto pr-1 h-full", activeTab !== "output" && "hidden lg:flex")}>
            ...
            {/* Header Card */}
            <div className="border-l-2 border-l-rose-500 border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/40 rounded-r-xl rounded-l-sm p-4 flex flex-col shrink-0">
              <div className="flex items-center justify-between mb-3">
                ...
                {headerObj && (
                  <span className="text-[10px] text-zinc-500 font-mono bg-zinc-200 dark:bg-zinc-800 px-1.5 py-0.5 rounded border border-zinc-300 dark:border-zinc-700">Algorithm & Token Type</span>
                )}
              </div>
              <div className="font-mono text-[13px] text-zinc-800 dark:text-zinc-200 bg-white dark:bg-[#09090B] p-3 rounded-lg border border-zinc-200 dark:border-zinc-800/50 overflow-x-auto shadow-sm">
                {headerObj ? <JsonViewer data={headerObj} /> : <span className="text-zinc-400 italic">No header data</span>}
              </div>
            </div>

            {/* Payload Card */}
            <div className="border-l-2 border-l-purple-500 border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/40 rounded-r-xl rounded-l-sm p-4 flex flex-col flex-1 min-h-[220px]">
              ...
              <div className="font-mono text-[13px] text-zinc-800 dark:text-zinc-200 bg-white dark:bg-[#09090B] p-3 rounded-lg border border-zinc-200 dark:border-zinc-800/50 flex-1 overflow-y-auto shadow-sm">
            ...
            <div className="border-l-2 border-l-sky-500 border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/40 rounded-r-xl rounded-l-sm p-4 flex flex-col shrink-0">
              ...
              <div className="flex flex-col gap-3">
                <div className="font-mono text-xs text-zinc-500 break-all bg-white dark:bg-[#09090B] p-3 rounded-lg border border-zinc-200 dark:border-zinc-800/50 shadow-sm">
                  {signature || "No signature"}
                </div>
                
                {headerObj?.alg?.startsWith('HS') && (
                  <div className="flex items-center gap-3 bg-white dark:bg-[#09090B] p-2 rounded-lg border border-zinc-200 dark:border-zinc-800/50 shadow-sm">
                    <input 
                      type="text"
                      value={secret}
                      onChange={e => setSecret(e.target.value)}
                      placeholder={`${headerObj.alg} Secret Key`}
                      className="flex-1 bg-transparent border-none outline-none text-xs font-mono text-zinc-900 dark:text-zinc-100 px-2 placeholder:text-zinc-400 dark:placeholder:text-zinc-600"
                    />
                    {sigStatus === "valid" && (
                      <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-500/20">
                        <Check className="w-3.5 h-3.5" />
                        <span className="text-xs font-semibold">Signature Verified</span>
                      </div>
                    )}
                    {sigStatus === "invalid" && secret.length > 0 && (
                      <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-red-50 dark:bg-red-500/10 text-red-600 dark:text-red-400 border border-red-200 dark:border-red-500/20">
