      ...
      <div className="flex flex-wrap items-center">
        {Array.from({ length: 32 }).map((_, i) => (
          <span key={i} className="flex items-center">
            {i > 0 && i % 8 === 0 && <span className="text-zinc-300 dark:text-zinc-700 font-normal mx-0.5 md:mx-1">.</span>}
            <span className={cn(
              "font-mono text-sm sm:text-base md:text-lg tracking-widest",
              i < prefix ? "text-zinc-900 dark:text-zinc-100 font-bold" : "text-zinc-400 dark:text-zinc-500 font-normal"
            )}>
              {bits[i]}
            </span>
          </span>
    ...
    <div className="flex flex-col h-full bg-[#09090B] overflow-y-auto w-full overflow-x-hidden relative">
      ...
      <div className="min-h-14 border-b border-zinc-200 dark:border-zinc-800 flex items-center justify-between gap-2 overflow-x-auto no-scrollbar py-2 px-3 md:px-4 bg-white dark:bg-zinc-950 shrink-0 sticky top-0 z-10">
        <div className="flex items-center gap-2">
          <Network className="w-4 h-4 text-zinc-900 dark:text-zinc-100" />
          <h2 className="text-sm font-semibold text-zinc-900 dark:text-zinc-100">CIDR Calculator</h2>
        </div>
      ...
      <div className="p-4 md:p-6 lg:p-8 max-w-5xl mx-auto w-full flex-1 flex flex-col gap-6 md:gap-8 bg-white dark:bg-[#09090B]">
        
        {/* CIDR Input & Quick Preset Strip */}
        <div className="flex flex-col gap-3">
          <label className="block text-xs font-semibold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">CIDR Notation</label>
          <input
            type="text"
            value={input}
            onChange={(e) => setInput(e.target.value)}
            placeholder="e.g. 192.168.1.0/24"
            className="w-full font-mono text-lg font-bold bg-zinc-50 dark:bg-zinc-900/50 border border-zinc-200 dark:border-zinc-800 rounded-xl p-4 text-zinc-900 dark:text-zinc-100 focus:ring-1 focus:ring-zinc-400 focus:outline-none placeholder:text-zinc-400 dark:placeholder:text-zinc-600 transition-all"
          />
          <div className="flex items-center gap-2 overflow-x-auto no-scrollbar pb-1">
            {['/8', '/16', '/24', '/27', '/30', '/32'].map((mask) => (
              <button
                ...
                className="shrink-0 px-3 py-1.5 rounded-full text-xs font-medium border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900/50 text-zinc-600 dark:text-zinc-300 hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors"
              >
                {mask}
              </button>
            ))}
        ...
        {info ? (
          <>
            ...
            <div className="mt-4 bg-zinc-50 dark:bg-zinc-900/30 border border-zinc-200 dark:border-zinc-800 rounded-xl p-5 md:p-6 shadow-sm overflow-hidden flex flex-col gap-6">
              <div className="flex items-center justify-between">
                <span className="text-xs font-semibold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">Binary Subnet Visualizer</span>
                <div className="flex items-center gap-4 text-[11px] font-medium uppercase tracking-wider">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-zinc-600 dark:text-zinc-400">Network Bits</span>
                  </div>
                  <div className="flex items-center gap-1.5">
                    <span className="w-2 h-2 rounded-full border border-zinc-400 dark:border-zinc-500"></span>
              ...
              <div className="flex flex-col gap-4 overflow-x-auto no-scrollbar pb-2">
                <div className="flex flex-col gap-1.5">
                  <span className="text-xs text-zinc-500 dark:text-zinc-400 font-medium">IP Address</span>
                  {renderBits(info.ip)}
                </div>
                <div className="flex flex-col gap-1.5">
                  <span className="text-xs text-zinc-500 dark:text-zinc-400 font-medium">Netmask</span>
          ...
          <div className="text-center py-20 text-zinc-400 dark:text-zinc-500 text-sm border border-dashed border-zinc-200 dark:border-zinc-800 rounded-xl bg-zinc-50 dark:bg-zinc-900/20">
...
function MetricCard({ label, value, id, copiedId, onCopy }: { label: string; value: string | number; id: string; copiedId: string | null; onCopy: (val: string, id: string) => void }) {
  return (
    <div className="bg-white dark:bg-zinc-900/40 border border-zinc-200 dark:border-zinc-800 rounded-xl p-4 shadow-sm relative group hover:border-zinc-300 dark:hover:border-zinc-700 transition-all flex flex-col justify-center min-h-[90px]">
      <div className="flex items-center justify-between mb-1.5">
        <span className="text-xs font-semibold tracking-wider text-zinc-500 dark:text-zinc-400 uppercase">{label}</span>
        <button 
          onClick={() => onCopy(value.toString(), id)}
          className="p-1 rounded-md opacity-0 group-hover:opacity-100 transition-opacity bg-zinc-100 dark:bg-zinc-800 hover:bg-zinc-200 dark:hover:bg-zinc-700 focus:opacity-100"
          title="Copy value"
        >
          {copiedId === id ? (
            <Check className="w-3.5 h-3.5 text-zinc-900 dark:text-zinc-100" />
          ) : (
            <Copy className="w-3.5 h-3.5 text-zinc-500 hover:text-zinc-900 dark:hover:text-zinc-100 transition-colors" />
          )}
        </button>
      </div>
      <span className="font-mono text-base font-bold text-zinc-900 dark:text-zinc-100 select-all pr-6">{value}</span>
    </div>
  );
}
