...
export function EmbedButton({ toolSlug, data, className }: EmbedButtonProps) {
 ...
 const embedUrl = `${origin}/tools/${toolSlug}?embed=true#data=${encoded}`;
...
"bg-transparent hover:bg-zinc-100 text-zinc-600 rounded-md h-8 w-8 flex items-center justify-center transition-colors shrink-0",
 ...
 <div className="flex items-center justify-between px-3.5 py-2 bg-zinc-200/60 border-b border-zinc-200">
 ...
 <pre className="p-3.5 text-xs font-mono text-zinc-800 whitespace-pre-wrap break-all overflow-x-auto max-h-56 sm:max-h-64 overflow-y-auto leading-relaxed">
 {getEmbedCode()}
 </pre>
 </div>
 </div>
